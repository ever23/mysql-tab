...
await usuarios.select({ "estado": "activo", "||nombre%": "Anti%" });
```
...
Esta es la forma más segura y clara de realizar consultas, evitando problemas de polimorfismo:

```javascript
await usuarios.select({
    where: { "id_usuario": 1, "tipo!": "admin" },
    order: "id DESC",
    limit: 10
});
```
